...
export const en: Record<string, string> = {
  ...
  'Zwrócone': 'Paid back',
  'Odejmij tę kwotę z salda konta - zaznacz tylko wtedy, gdy przy zakupie konto zostało pominięte przez pomyłkę. Domyślnie zapisujemy samo powiązanie.': 'Subtract this amount from the account balance - tick this only if you forgot to pick the account when you bought it. By default we just record the link.',
  'Nie udało się odczytać sumy z paragonu - liczymy ją z pozycji poniżej. Sprawdź, czy się zgadza.': 'We could not read the total from the receipt, so we added up the items below. Check that it matches.',
  'Podziału dokonał prostszy model - warto sprawdzić pozycje.': 'A simpler model split this receipt, so the items may need a few corrections.',
  'Dzienny limit mocniejszego modelu wyczerpany - paragon odczytany prościej, jedną kategorią. Spróbuj jutro.': 'The stronger model\'s daily allowance is used up, so the receipt was read as a single category. Try again tomorrow.',
  'Nie udało się rozbić tego paragonu na pozycje - odczytaliśmy go jedną kwotą. Spróbuj wyraźniejszego zdjęcia.': 'This receipt could not be split into items, so we read it as one amount. Try a clearer photo.',
  'Klucz jest Twój i działa w oparciu o Twoje konto Google - to Google ustala darmowe limity i ewentualne opłaty po ich przekroczeniu, więc za rozliczenia po stronie Google nie odpowiadamy. Klucz trzymamy zaszyfrowany i usuniesz go stąd jednym kliknięciem.': 'The key is yours and runs on your own Google account - Google sets the free limits and any charges beyond them, so billing on their side is not something we can answer for. We store the key encrypted and you can remove it here in one click.',
  'Szczegóły w regulaminie': 'Details in the terms',
  'Skąd przyszły konta': 'Where accounts came from',
  'Liczone przy rejestracji, nie przy kliknięciu - to konta, które faktycznie powstały.': 'Counted at registration, not at the click - these are accounts that actually got created.',
  'Konta z artykułów': 'Accounts from articles',
  'Który tekst realnie przyprowadził użytkownika, a nie tylko czytelnika.': 'Which piece brought in a user, not just a reader.',
  'Strona główna (góra)': 'Home page (top)',
  'Strona główna (FAQ)': 'Home page (FAQ)',
  'Artykuł': 'Article',
  ...
  'Zaznacz kategorie, które chcesz porównywać - reszta na razie się schowa, żeby zacząć od czytelnego widoku zamiast dwudziestu wierszy naraz. Nic z tego nie jest ostateczne: pojedyncze wiersze nadal odznaczysz i dodasz poniżej.': 'Pick the categories you want to compare - the rest will hide for now, so you start with a readable view instead of twenty rows at once. Nothing here is final: you can still tick individual rows off or add them back below.',
  'Pokaż wybrane': 'Show the ones I picked',
  'Pokaż mi wszystko': 'Show me everything',
  'Zmień, co Cię interesuje': 'Change what interests you',
  'za rok {0}': 'for {0}',
  'dzień dywidendy {0}': 'record date {0}',
  'wypłata {0}': 'payment {0}',
  'Dywidenda na akcję': 'Dividend per share',
  'Najbliższe wyniki': 'Next results',
  'Walne zgromadzenie': 'General meeting',
  'Średnia cena docelowa': 'Average target price',
  'Konsensus analityków': 'Analyst consensus',
  'Najważniejsze dane': 'Key figures',
  'Zapamiętane z Twoich analiz - każda wartość pochodzi z najnowszej analizy, która ją znalazła.': 'Remembered from your analyses - each value comes from the most recent analysis that found it.',
  'wg analizy z {0}': 'per the analysis of {0}',
  'Stan na {0}': 'As of {0}',
  'źródła: {0} (komunikaty spółki i wiadomości)': 'sources: {0} (company filings and news)',
  'nie znalazłem żadnych komunikatów ani wiadomości o tej spółce z tego okresu': 'no filings or news about this company were found for this period',
  'Komunikaty ESPI/EBI': 'ESPI/EBI filings',
  'Pozostałe zebrane materiały, które nie weszły do podsumowania ({0})': 'Other gathered material that did not make it into the summary ({0})',
  'Nic nowego w tym okresie.': 'Nothing new in this period.',
  'Brak informacji.': 'No information.',
  'Źródła ({0})': 'Sources ({0})',
  'Ta spółka była już dziś sprawdzana ({0}). Każde sprawdzenie zużywa zapytanie z darmowego dziennego limitu Twojego klucza Gemini - tego samego, z którego korzysta skanowanie paragonów. Gdy się wyczerpie, oba działają dopiero po odnowieniu limitu, około 9:00 czasu polskiego. Na pewno sprawdzić jeszcze raz?': 'This company was already checked today ({0}). Every check uses a request from the free daily limit of your Gemini key - the same one receipt scanning uses. Once it runs out, both only work again after the limit resets, around 9:00 Polish time. Check again anyway?',
  'Co dzieje się w wybranej spółce: nadchodzące daty, komunikaty, transakcje, wyniki, plotki, oczekiwania i rekomendacje analityków. skieta zbiera komunikaty spółki i wiadomości z serwisów finansowych, a Google Gemini streszcza je w punktach na Twoim własnym kluczu - przy każdym punkcie jest numer źródła.': 'What is happening at the selected company: upcoming dates, filings, transactions, results, rumours, expectations and analyst recommendations. skieta gathers the company\'s filings and financial news, and Google Gemini summarises them in bullet points on your own key - every point carries the number of its source.',
  'To nie jest rekomendacja inwestycyjna. Podsumowanie pisze model językowy, który może się pomylić albo coś pominąć - zanim na jego podstawie cokolwiek zrobisz, sprawdź informację w źródle.': 'This is not investment advice. The summary is written by a language model that can make mistakes or miss things - check the source before you act on anything in it.',
  'Nie masz jeszcze w portfelu żadnej spółki do przeanalizowania.': 'You don\'t have any company in your portfolio to analyse yet.',
  'Przejdź do portfela': 'Go to portfolio',
  'Informacje z': 'News from',
  'Ostatnio sprawdzane: {0}': 'Last checked: {0}',
  'Jeszcze nie sprawdzano': 'Not checked yet',
  'Analiza potrzebuje Twojego klucza Gemini': 'The analysis needs your Gemini key',
  'Komunikaty i wiadomości o spółce streszcza Google Gemini, na Twoim własnym, darmowym kluczu - tym samym, którego używa skanowanie paragonów. Ustawisz go raz, w dwie minuty, bez karty płatniczej.': 'Filings and news about the company are summarised by Google Gemini on your own free key - the same one receipt scanning uses. You set it up once, in two minutes, with no payment card.',
  'Zbieram komunikaty i wiadomości o spółce i przygotowuję podsumowanie… Zwykle trwa to do minuty - możesz w tym czasie przejść do innej zakładki.': 'Gathering the company\'s filings and news and preparing the summary… This usually takes up to a minute - feel free to switch to another tab meanwhile.',
  'Poniżej ostatnia udana analiza.': 'The last successful analysis is shown below.',
  'Kliknij „Sprawdź teraz”, żeby przygotować pierwszą analizę tej spółki.': 'Click "Check now" to prepare the first analysis of this company.',
  'Dziś': 'Today',
  'Ten tydzień': 'This week',
  'Najważniejsze nadchodzące daty': 'Key upcoming dates',
  'Ważne transakcje': 'Notable transactions',
  'Wyniki finansowe': 'Financial results',
  'Plotki i niepotwierdzone doniesienia': 'Rumours and unconfirmed reports',
  'Oczekiwania wobec spółki': 'Expectations for the company',
  'Rekomendacje analityków': 'Analyst recommendations',
  'Analiza spółki': 'Company analysis',
  'Wyczerpał się darmowy dzienny limit zapytań Twojego klucza Gemini. Limit odnawia się codziennie około 9:00 czasu polskiego - wtedy spróbuj ponownie.': 'The free daily request limit of your Gemini key has run out. It resets every day around 9:00 Polish time - try again then.',
  'Google odrzucił Twój klucz Gemini. Sprawdź go albo wklej nowy w ustawieniach konta.': 'Google rejected your Gemini key. Check it or paste a new one in your account settings.',
  'Nie ustawiono klucza Gemini.': 'No Gemini key has been set.',
  'Sprawdzanie trwało za długo i zostało przerwane. Spróbuj ponownie za chwilę.': 'The check took too long and was stopped. Try again in a moment.',
  'Nie udało się przygotować analizy. Spróbuj ponownie za chwilę.': 'The analysis could not be prepared. Try again in a moment.',
  'rekomendacja zarządu': 'management\'s proposal',
  'uchwalona przez walne zgromadzenie': 'approved by the general meeting',
  'Wiadomości': 'News',
  'Otwieram demo…': 'Opening the demo…',
  ...
  'Tak. Przycisk „Zobacz demo" na górze strony otwiera przykładowe konto z budżetem, portfelem, celami oszczędnościowymi i samochodem - możesz wszystko przeklikać, tylko zmiany nie są zapisywane. Bez konta działa też kalkulator inwestycyjny.': 'Yes. The “See the demo” button at the top of the page opens a sample account with a budget, a portfolio, savings goals and a car - you can click through everything, only changes are not saved. The investment calculator works without an account too.',
  ...
  'Wyjdź z demo': 'Leave the demo',
  'Wpłata co miesiąc': 'Monthly contribution',
  'Założona inflacja (% rocznie)': 'Assumed inflation (% a year)',
  'Wpłacisz łącznie {0}. Inflacja wycenia obligacje indeksowane od drugiego roku i przelicza wynik na dzisiejsze pieniądze - cel NBP to 2,5%.': 'You will pay in {0} in total. Inflation sets the rate of the index-linked bonds from their second year and converts the result into today’s money - the NBP target is 2.5%.',
  'Wartość realna (po inflacji)': 'Real value (after inflation)',
  'Przywróć domyślną stawkę ({0}%)': 'Restore the default rate ({0}%)',
  'od 2. roku: {0}%': 'from year 2: {0}%',
  'Pierwszy rok: stałe oprocentowanie. Od drugiego roku: inflacja + marża {0}% - kalkulator liczy to z założonej inflacji, którą ustawiasz powyżej. Rzeczywisty wynik zależy od tego, jaka inflacja faktycznie będzie.': 'First year: a fixed rate. From the second year: inflation + a {0}% margin - the calculator works this out from the inflation you assume above. The real result depends on what inflation actually turns out to be.',
  'Odsetki z COI są wypłacane co roku - wynik zakłada, że je reinwestujesz.': 'COI interest is paid out every year - the result assumes you reinvest it.',
  'Wydatki w skiecie na telefonie, z przyciskiem skanowania paragonu w nagłówku': 'Expenses in skieta on a phone, with the receipt scan button in the header',
  'Raporty dla SEC': 'SEC filings',
  'Komunikaty ESPI/EBI (GPW), raporty dla SEC i ważne newsy (USA) dla spółek z Twojego portfela - sprawdzane raz dziennie.': 'ESPI/EBI filings (WSE), SEC filings and key news (USA) for the companies in your portfolio - checked once a day.',
  'Podobne spółki': 'Similar companies',
  'Spółki z tej samej branży zaproponowane przez model i sprawdzone w wyszukiwarce notowań - to nie jest rekomendacja.': 'Companies from the same industry suggested by the model and checked against the stock search - this is not a recommendation.',
  'Analizuj': 'Analyse',
  'Dashboard skiety na telefonie': 'The skieta dashboard on a phone',
  ...
  'Kwota na start': 'Starting amount',
  ...
  'Zaloguj się, żeby dodać dowolną spółkę z realną, historyczną stopą zwrotu.':
    'Log in to add any company with a real, historical rate of return.',
  'Śledź to na bieżąco, nie tylko dziś':
    'Track this on an ongoing basis, not just today',
  'skieta liczy to samo dla Twojego prawdziwego portfela - konta bankowe, akcje, obligacje i lokaty w jednym miejscu, z zyskiem po podatku Belki. Dostęp jest na zaproszenie - zostaw e-mail, a odezwiemy się.':
    "skieta calculates the same thing for your real portfolio - bank accounts, stocks, bonds and deposits in one place, with the after-tax gain worked out for you. Access is by invitation - leave your e-mail and we'll get back to you.",
  'Kalkulator inwestycyjny - lokata, obligacje czy giełda':
    'Investment calculator - savings, bonds or the stock market',
  ...
  'Przypomnij mi e-mailem, jeśli dawno się nie logowałem/am':
    'Remind me by email if I have not logged in for a while',
  ...
  'Przestaniemy wysyłać e-maile z przypomnieniem, gdy dawno się nie logowałeś/aś. Możesz to zmienić w każdej chwili w ustawieniach konta.':
    'We will stop sending you reminder emails about being inactive. You can change this anytime in your account settings.',
  'Tak, wypisz mnie':
    'Yes, unsubscribe me',
  'Wypisywanie…':
    'Unsubscribing…',
  ...
  'Nie udało się wypisać z przypomnień.':
    'Could not unsubscribe from reminders.',
  ...
  'Dywidendy YTD': 'Dividends YTD',

  // Admin: articles visibility per language
  'Artykuły': 'Articles',
  'Widoczność artykułów': 'Articles visibility',
