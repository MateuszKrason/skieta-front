...
export const es: Record<string, string> = {
  ...
  'Zwrócone': 'Devuelto',
  'Odejmij tę kwotę z salda konta - zaznacz tylko wtedy, gdy przy zakupie konto zostało pominięte przez pomyłkę. Domyślnie zapisujemy samo powiązanie.': 'Resta este importe del saldo de la cuenta: márcalo solo si olvidaste elegir la cuenta al comprar. Por defecto guardamos solo la vinculación.',
  'Nie udało się odczytać sumy z paragonu - liczymy ją z pozycji poniżej. Sprawdź, czy się zgadza.': 'No pudimos leer el total del recibo, así que lo sumamos de las líneas de abajo. Comprueba que cuadre.',
  'Podziału dokonał prostszy model - warto sprawdzić pozycje.': 'Este recibo lo dividió un modelo más sencillo: puede que las líneas necesiten retoques.',
  'Dzienny limit mocniejszego modelu wyczerpany - paragon odczytany prościej, jedną kategorią. Spróbuj jutro.': 'El límite diario del modelo más potente se ha agotado: el recibo se leyó como una sola categoría. Inténtalo mañana.',
  'Nie udało się rozbić tego paragonu na pozycje - odczytaliśmy go jedną kwotą. Spróbuj wyraźniejszego zdjęcia.': 'No se pudo dividir este recibo en líneas, así que lo leímos como un único importe. Prueba con una foto más nítida.',
  'Klucz jest Twój i działa w oparciu o Twoje konto Google - to Google ustala darmowe limity i ewentualne opłaty po ich przekroczeniu, więc za rozliczenia po stronie Google nie odpowiadamy. Klucz trzymamy zaszyfrowany i usuniesz go stąd jednym kliknięciem.': 'La clave es tuya y funciona en tu propia cuenta de Google: Google fija los límites gratuitos y los posibles cargos al superarlos, así que no respondemos por la facturación de su lado. Guardamos la clave cifrada y puedes borrarla aquí con un clic.',
  'Szczegóły w regulaminie': 'Detalles en los términos',
  'Skąd przyszły konta': 'De dónde vinieron las cuentas',
  'Liczone przy rejestracji, nie przy kliknięciu - to konta, które faktycznie powstały.': 'Contadas al registrarse, no al hacer clic: son cuentas que realmente se crearon.',
  'Konta z artykułów': 'Cuentas desde artículos',
  'Który tekst realnie przyprowadził użytkownika, a nie tylko czytelnika.': 'Qué texto trajo de verdad a un usuario, no solo a un lector.',
  'Strona główna (góra)': 'Página principal (arriba)',
  'Strona główna (FAQ)': 'Página principal (FAQ)',
  'Artykuł': 'Artículo',
  ...
  'Czym się interesujesz?': '¿Qué te interesa?',
  'Zaznacz kategorie, które chcesz porównywać - reszta na razie się schowa, żeby zacząć od czytelnego widoku zamiast dwudziestu wierszy naraz. Nic z tego nie jest ostateczne: pojedyncze wiersze nadal odznaczysz i dodasz poniżej.': 'Marca las categorías que quieres comparar - el resto se ocultará por ahora, para empezar con una vista clara en lugar de veinte filas a la vez. Nada de esto es definitivo: seguirás pudiendo quitar o añadir filas individuales más abajo.',
  'Pokaż wybrane': 'Mostrar las elegidas',
  'Pokaż mi wszystko': 'Muéstrame todo',
  'Zmień, co Cię interesuje': 'Cambiar lo que te interesa',
  'za rok {0}': 'del ejercicio {0}',
  'dzień dywidendy {0}': 'fecha de registro {0}',
  'wypłata {0}': 'pago {0}',
  'Dywidenda na akcję': 'Dividendo por acción',
  'Najbliższe wyniki': 'Próximos resultados',
  'Walne zgromadzenie': 'Junta general',
  'Średnia cena docelowa': 'Precio objetivo medio',
  'Konsensus analityków': 'Consenso de analistas',
  'Najważniejsze dane': 'Datos clave',
  'Zapamiętane z Twoich analiz - każda wartość pochodzi z najnowszej analizy, która ją znalazła.': 'Guardado de tus análisis: cada valor procede del análisis más reciente que lo encontró.',
  'wg analizy z {0}': 'según el análisis del {0}',
  'Stan na {0}': 'A fecha de {0}',
  'źródła: {0} (komunikaty spółki i wiadomości)': 'fuentes: {0} (comunicados de la empresa y noticias)',
  'nie znalazłem żadnych komunikatów ani wiadomości o tej spółce z tego okresu': 'no se encontraron comunicados ni noticias sobre esta empresa en este periodo',
  'Komunikaty ESPI/EBI': 'Comunicados ESPI/EBI',
  'Pozostałe zebrane materiały, które nie weszły do podsumowania ({0})': 'Otros materiales reunidos que no entraron en el resumen ({0})',
  'Nic nowego w tym okresie.': 'Nada nuevo en este periodo.',
  'Brak informacji.': 'Sin información.',
  'Źródła ({0})': 'Fuentes ({0})',
  'Ta spółka była już dziś sprawdzana ({0}). Każde sprawdzenie zużywa zapytanie z darmowego dziennego limitu Twojego klucza Gemini - tego samego, z którego korzysta skanowanie paragonów. Gdy się wyczerpie, oba działają dopiero po odnowieniu limitu, około 9:00 czasu polskiego. Na pewno sprawdzić jeszcze raz?': 'Esta empresa ya se ha comprobado hoy ({0}). Cada comprobación consume una solicitud del límite diario gratuito de tu clave de Gemini, el mismo que usa el escaneo de tickets. Cuando se agota, ambas funciones vuelven a funcionar solo cuando se renueva el límite, hacia las 9:00 hora de Polonia. ¿Comprobar de nuevo?',
  'Co dzieje się w wybranej spółce: nadchodzące daty, komunikaty, transakcje, wyniki, plotki, oczekiwania i rekomendacje analityków. skieta zbiera komunikaty spółki i wiadomości z serwisów finansowych, a Google Gemini streszcza je w punktach na Twoim własnym kluczu - przy każdym punkcie jest numer źródła.': 'Qué está pasando en la empresa elegida: próximas fechas, comunicados, transacciones, resultados, rumores, expectativas y recomendaciones de analistas. skieta reúne los comunicados de la empresa y noticias financieras, y Google Gemini los resume en puntos con tu propia clave; cada punto lleva el número de su fuente.',
  'To nie jest rekomendacja inwestycyjna. Podsumowanie pisze model językowy, który może się pomylić albo coś pominąć - zanim na jego podstawie cokolwiek zrobisz, sprawdź informację w źródle.': 'Esto no es una recomendación de inversión. El resumen lo escribe un modelo de lenguaje que puede equivocarse u omitir algo: antes de actuar, comprueba la información en la fuente.',
  'Nie masz jeszcze w portfelu żadnej spółki do przeanalizowania.': 'Todavía no tienes ninguna empresa en tu cartera para analizar.',
  'Przejdź do portfela': 'Ir a la cartera',
  'Informacje z': 'Información de',
  'Ostatnio sprawdzane: {0}': 'Última comprobación: {0}',
  'Jeszcze nie sprawdzano': 'Aún sin comprobar',
  'Analiza potrzebuje Twojego klucza Gemini': 'El análisis necesita tu clave de Gemini',
  'Komunikaty i wiadomości o spółce streszcza Google Gemini, na Twoim własnym, darmowym kluczu - tym samym, którego używa skanowanie paragonów. Ustawisz go raz, w dwie minuty, bez karty płatniczej.': 'Google Gemini resume los comunicados y noticias de la empresa con tu propia clave gratuita, la misma que usa el escaneo de tickets. La configuras una sola vez, en dos minutos y sin tarjeta de pago.',
  'Zbieram komunikaty i wiadomości o spółce i przygotowuję podsumowanie… Zwykle trwa to do minuty - możesz w tym czasie przejść do innej zakładki.': 'Reuniendo comunicados y noticias de la empresa y preparando el resumen… Suele tardar hasta un minuto; mientras tanto puedes ir a otra pestaña.',
  'Poniżej ostatnia udana analiza.': 'Abajo tienes el último análisis correcto.',
  'Kliknij „Sprawdź teraz”, żeby przygotować pierwszą analizę tej spółki.': 'Pulsa «Comprobar ahora» para preparar el primer análisis de esta empresa.',
  'Dziś': 'Hoy',
  'Ten tydzień': 'Esta semana',
  'Najważniejsze nadchodzące daty': 'Próximas fechas clave',
  'Ważne transakcje': 'Transacciones relevantes',
  'Wyniki finansowe': 'Resultados financieros',
  'Plotki i niepotwierdzone doniesienia': 'Rumores y noticias sin confirmar',
  'Oczekiwania wobec spółki': 'Expectativas sobre la empresa',
  'Rekomendacje analityków': 'Recomendaciones de analistas',
  'Analiza spółki': 'Análisis de la empresa',
  'Wyczerpał się darmowy dzienny limit zapytań Twojego klucza Gemini. Limit odnawia się codziennie około 9:00 czasu polskiego - wtedy spróbuj ponownie.': 'Se ha agotado el límite diario gratuito de solicitudes de tu clave de Gemini. Se renueva cada día hacia las 9:00 hora de Polonia; vuelve a intentarlo entonces.',
  'Google odrzucił Twój klucz Gemini. Sprawdź go albo wklej nowy w ustawieniach konta.': 'Google ha rechazado tu clave de Gemini. Compruébala o pega una nueva en los ajustes de la cuenta.',
  'Nie ustawiono klucza Gemini.': 'No se ha configurado ninguna clave de Gemini.',
  'Sprawdzanie trwało za długo i zostało przerwane. Spróbuj ponownie za chwilę.': 'La comprobación tardó demasiado y se interrumpió. Inténtalo de nuevo en un momento.',
  'Nie udało się przygotować analizy. Spróbuj ponownie za chwilę.': 'No se pudo preparar el análisis. Inténtalo de nuevo en un momento.',
  'rekomendacja zarządu': 'propuesta del consejo',
  'uchwalona przez walne zgromadzenie': 'aprobado por la junta general',
  'Wiadomości': 'Noticias',
  'Otwieram demo…': 'Abriendo la demo…',
  ...
  'Tak. Przycisk „Zobacz demo" na górze strony otwiera przykładowe konto z budżetem, portfelem, celami oszczędnościowymi i samochodem - możesz wszystko przeklikać, tylko zmiany nie są zapisywane. Bez konta działa też kalkulator inwestycyjny.': 'Sí. El botón «Ver la demo» de la parte superior abre una cuenta de ejemplo con presupuesto, cartera, metas de ahorro y coche: puedes recorrerlo todo, solo que los cambios no se guardan. La calculadora de inversiones también funciona sin cuenta.',
  ...
  'Wyjdź z demo': 'Salir de la demo',
  'Wpłata co miesiąc': 'Aportación mensual',
  'Założona inflacja (% rocznie)': 'Inflación supuesta (% anual)',
  'Wpłacisz łącznie {0}. Inflacja wycenia obligacje indeksowane od drugiego roku i przelicza wynik na dzisiejsze pieniądze - cel NBP to 2,5%.': 'Aportarás {0} en total. La inflación determina el interés de los bonos indexados desde el segundo año y convierte el resultado a dinero de hoy; el objetivo del NBP es el 2,5%.',
  'Wartość realna (po inflacji)': 'Valor real (descontada la inflación)',
  'Przywróć domyślną stawkę ({0}%)': 'Restablecer el tipo predeterminado ({0}%)',
  'od 2. roku: {0}%': 'desde el 2.º año: {0}%',
  'Pierwszy rok: stałe oprocentowanie. Od drugiego roku: inflacja + marża {0}% - kalkulator liczy to z założonej inflacji, którą ustawiasz powyżej. Rzeczywisty wynik zależy od tego, jaka inflacja faktycznie będzie.': 'Primer año: tipo fijo. Desde el segundo año: inflación + un margen del {0}%; la calculadora lo obtiene de la inflación que supones arriba. El resultado real depende de la inflación que haya de verdad.',
  'Odsetki z COI są wypłacane co roku - wynik zakłada, że je reinwestujesz.': 'Los intereses de los COI se pagan cada año; el resultado supone que los reinviertes.',
  'Wydatki w skiecie na telefonie, z przyciskiem skanowania paragonu w nagłówku': 'Gastos en skieta en el móvil, con el botón para escanear tickets en la cabecera',
  'Raporty dla SEC': 'Informes a la SEC',
  'Komunikaty ESPI/EBI (GPW), raporty dla SEC i ważne newsy (USA) dla spółek z Twojego portfela - sprawdzane raz dziennie.': 'Comunicados ESPI/EBI (GPW), informes a la SEC y noticias importantes (EE. UU.) de las empresas de tu cartera; se comprueban una vez al día.',
  'Podobne spółki': 'Empresas similares',
  'Spółki z tej samej branży zaproponowane przez model i sprawdzone w wyszukiwarce notowań - to nie jest rekomendacja.': 'Empresas del mismo sector propuestas por el modelo y comprobadas en el buscador de cotizaciones; no es una recomendación.',
  'Analizuj': 'Analizar',
  'Dashboard skiety na telefonie': 'El panel de skieta en el móvil',
  ...
  'Kwota na start': 'Importe inicial',
  ...
  'Zaloguj się, żeby dodać dowolną spółkę z realną, historyczną stopą zwrotu.':
    'Inicia sesión para añadir cualquier empresa con una rentabilidad histórica real.',
  'Śledź to na bieżąco, nie tylko dziś':
    'Sigue esto de forma continua, no solo hoy',
  'skieta liczy to samo dla Twojego prawdziwego portfela - konta bankowe, akcje, obligacje i lokaty w jednym miejscu, z zyskiem po podatku Belki. Dostęp jest na zaproszenie - zostaw e-mail, a odezwiemy się.':
    'skieta calcula lo mismo para tu cartera real - cuentas bancarias, acciones, bonos y depósitos en un solo lugar, con la ganancia ya calculada después de impuestos. El acceso es por invitación - déjanos tu correo y te contactaremos.',
  'Kalkulator inwestycyjny - lokata, obligacje czy giełda':
    'Calculadora de inversión - depósito, bonos o bolsa',
